...
?>     <tr>
    <td> 
      Jemaat ulang tahun pada tanggal <?php echo "$BirthDay1-$BirthMonth1-$BirthYear1 "; ?> 
    </td>
  </tr>
  <tr>
...
<? if ($_GET[action]=='costum') { ?>   
   ...
   $SQL = "SELECT * from person_per LEFT JOIN family_fam ON  family_fam.fam_ID = person_per.per_fam_ID where per_BirthDay=$BirthDay1 AND per_BirthMonth=$BirthMonth1 AND per_Cls_ID=1";
   $TotalFemaleListing = mysql_query($SQL); 
     while ($Hasil=mysql_fetch_array($TotalFemaleListing))
        ...
        <tr bgcolor="<? echo($bg); ?>" onmouseover="bgColor='#FFFFCC'" onmouseout="bgColor='<? echo($bg); ?>'" > 
          ...
          <td>&nbsp;<? echo"<a href='PersonView.php?PersonID=$Hasil[per_ID]'>$Hasil[per_FirstName] $Hasil[per_MiddleName] $Hasil[per_LastName] (Kel. $Hasil[fam_Name])</td>";?></td>
   		  
		  <td width="9%" align="center">
		  <? $Ultahke=$TAHUNINI-$Hasil[per_BirthYear]; echo"$Ultahke";?></td>
          <td width="7%" align="center"><? echo"$Hasil[fam_NamaSek]";?></td>
        </tr>
        <?
	$No++;
       $n++;
