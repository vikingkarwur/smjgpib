...
else
{
	...
	{
		$headerString = "\"Nama Belakang\",";
		if (!empty($_POST["Title"])) $headerString .= "\"Gelar\",";
		if (!empty($_POST["FirstName"])) $headerString .= "\"Nama Depan\",";
		if (!empty($_POST["Suffix"])) $headerString .= "\"Gelar\",";
		if (!empty($_POST["MiddleName"])) $headerString .= "\"Nama Tengah\",";
	}

	if (!empty($_POST["NoInd"])) $headerString .= "\"No Induk\",";
	if (!empty($_POST["Sektor"])) $headerString .= "\"Sektor Pelayanan\",";
	if (!empty($_POST["Address1"])) $headerString .= "\"Alamat 1\",";
	if (!empty($_POST["Address2"])) $headerString .= "\"Alamat 2\",";
	if (!empty($_POST["City"])) $headerString .= "\"Kota\",";
	if (!empty($_POST["State"])) $headerString .= "\"Propinsi\",";
	if (!empty($_POST["Zip"])) $headerString .= "\"Kode Pos\",";
	if (!empty($_POST["Country"])) $headerString .= "\"Negara\",";
	if (!empty($_POST["HomePhone"])) $headerString .= "\"Telepon Rumah\",";
	if (!empty($_POST["WorkPhone"])) $headerString .= "\"Telepon Kantor\",";
	if (!empty($_POST["CellPhone"])) $headerString .= "\"HandPhone\",";
	if (!empty($_POST["Email"])) $headerString .= "\"Email\",";
	if (!empty($_POST["WorkEmail"])) $headerString .= "\"Email Kantor\",";
	if (!empty($_POST["Envelope"])) $headerString .= "\"Envelope Number\",";
	if (!empty($_POST["MembershipDate"])) $headerString .= "\"Tanggal Terdaftar\",";


	if ($sFormat == "default")
	{
		if (!empty($_POST["BirthdayDate"])) $headerString .= "\"Tanggal Lahir\",";
		if (!empty($_POST["Age"])) $headerString .= "\"Umur\",";
		if (!empty($_POST["PrintFamilyRole"])) $headerString .= "\"Hubungan Keluarga\",";
	}
	else
	{
		if (!empty($_POST["BirthdayDate"])) $headerString .= "\"Tanggal Pernikahan\",";
	...
	while($aRow = mysql_fetch_array($rsLabelsToWrite))
	{
		...
		if (!($bSkipIncompleteAddr && (strlen($sCity) == 0 || strlen($sState) == 0 || strlen($sZip) == 0 || (strlen($sAddress1) == 0 && strlen($sAddress2) == 0))))
		{
			...
			if (!($bSkipNoEnvelope && (strlen($per_Envelope) == 0)))
			{
				...
				{
					if ($memberCount > 1)
						$sString = "\"Keluarga " . $fam_Name;
					else
						$sString = "\"" . $per_LastName . ", " . $per_FirstName;
				}

				if (isset($_POST["NoInd"])) $sString .= "\",\"" . $fam_NoIndFam . "-" . $per_NoIndJem . "." . $fam_NamaSek;
				if (isset($_POST["Sektor"])) $sString .= "\",\"" . $fam_NamaSek;
